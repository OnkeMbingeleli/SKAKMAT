...
(function () {
    ...
    async function loadLeaveBalance() {
        ...
        container.innerHTML = (res.data || []).map(s => `
            <div class="leave-balance-card ${Number(s.days_remaining) < 0 ? 'exceeded' : ''}">
                ...
                <div class="value">${s.unlimited ? 'Unlimited' : s.days_remaining} <small>${s.unlimited ? '' : `left / ${s.allocated_days}`}</small></div>
                <div class="muted">${s.unlimited ? 'No limit' : `${s.days_used} used`}</div>
            </div>
        `).join('');
    }
    ...
    async function loadAdminRequests() {
        const body = el('leaveRequestsBody');
        const emptyEl = el('leaveRequestsEmpty');
        body.innerHTML = '<tr><td colspan="8" style="text-align:center; color:var(--muted);">Loading…</td></tr>';
        ...
        body.innerHTML = rows.map(r => {
            ...
            const daysLeft = balance && balance.unlimited ? 'Unlimited'
                : balance ? `${escapeHtml(balance.days_remaining)} / ${escapeHtml(balance.allocated_days)}` : 'â€”';
            return `
                <tr>
