(() => {
    ...
    // Live clock for the admin header/hero (was stuck at "--:--:--" — nothing
    // was ever ticking it).
    function tickAdminClock() {
        const now = new Date();
        const time = now.toLocaleTimeString('en-ZA', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
        const date = new Intl.DateTimeFormat('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' }).format(now);

        const headerTime = document.getElementById('adminHeaderTime');
        const headerDate = document.getElementById('adminHeaderDate');
        const heroTime = document.getElementById('adminHeroTime');
        const heroDate = document.getElementById('adminHeroDate');

        if (headerTime) headerTime.firstChild.textContent = time + ' ';
        if (headerDate) headerDate.textContent = date;
        if (heroTime) heroTime.firstChild.textContent = time + ' ';
        if (heroDate) heroDate.textContent = date;
    }
    if (document.getElementById('adminHeaderTime') || document.getElementById('adminHeroTime')) {
        tickAdminClock();
